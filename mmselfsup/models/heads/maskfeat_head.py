...
class MaskFeatPretrainHead(BaseModule):
    ...
    def loss(self, pred, target, mask):
        ...
        Args:
            pred (torch.tensor): shape is (N, C, H, W).
            target (torch.tensor): shape is (N, C, H, W).
            mask (torch.tensor): shape is (N, H, W).
    ...
    def forward(self, latent, hog, mask):
        ...
        Args:
            latent (torch.tensor): shape is (N, L, C).
            hog (torch.tensor): shape is (N, C, H, W).
            mask (torch.tensor): shape is (N, L, C).
