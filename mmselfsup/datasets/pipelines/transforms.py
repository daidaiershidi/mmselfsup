...
class MaskfeatMaskGenerator(object):
    """Generate random block mask for each Image.
    This module is used in Maskfeat to generate masks.
    ...
    Args:
        ...
        min_num_patches (int): Maximum number of patches that require masking.
            Defaults to 15.
        max_num_patches (int, optional): Minimum number of patches that
            require masking.
        min_aspect (int): Maximum aspect of patches that require masking.
            Defaults to 0.3.
        max_aspect (float, optional): Minimum aspect of patches that
            require masking.
    """
    ...
    def __call__(self, img: torch.Tensor) -> Tuple[torch.Tensor, np.array]:
        """Generate single mask.

        Returns:
            np.array: Mask of shape (mask_window_size, mask_window_size).
        """
        mask = np.zeros(shape=self.get_shape(), dtype=np.int)
        mask_count = 0
        while mask_count < self.num_masking_patches:
